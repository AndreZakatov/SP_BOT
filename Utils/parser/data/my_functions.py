...
# Импорт данных из файла config.py
api_id = config.api_id
api_hash = config.api_hash
session = 'session.session'


# Асинхронная функция для отправки запроса на вступление в чат
async def inv_chat(link):
    hash = link.rsplit('/', 1)[1]
    async with TelegramClient(session, api_id, api_hash) as client:
        try:
            await client(functions.messages.ImportChatInviteRequest(hash=hash))
            res = await client(functions.messages.CheckChatInviteRequest(hash=hash))
            if res.chat.megagroup is False:
                print(
                    'Похоже, вы отправили ссылку на закрытый канал. Убедитесь, что вы собираете информацию из группы.')
                exit()
            ...
            print(
                'Срок действия чата, к которому пользователь пытался присоединиться, истек, и он больше не действителен.')
            exit()
...
# Асинхронная функция для проверки чата и сбора информации о его участниках
        ...
        try:
            ...
            if ch.__class__.__name__ == 'Channel':
                if ch.megagroup is False:
                    res = await client(functions.channels.GetFullChannelRequest(channel=ch))
                    if len(res.chats) != 2:
                ...
                if not aggressive:
                    ...
                    for num, x in enumerate(string.ascii_lowercase):
                        print(f'\rВыполняется расширенный поиск по алфавиту... {num}/{len(string.ascii_lowercase)} ',
                              end='')
                        m = 1
                        try:
                ...
                return {'status': 'ok', 'members': members, 'admins': admins,
                        'ch': ch, 'users.txt': users, 'channel_type': channel_type,
...
# Асинхронная функция для выгрузки сообщений из чата
...
# Функция для проверки типа ссылки
def check_link(link):
    try:
        if int(link):
            return 'id'
...
# Функция для создания словаря пользователей
def list_users(*args):
    members = args[0]
    users = {}
    for user in members:
